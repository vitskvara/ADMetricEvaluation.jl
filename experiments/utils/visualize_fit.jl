...
function visualize_fit(dataset, model, params, subclass, contamination, fpr, seed; 
	figname = "")
	# data acquisition
	p = 0.8
	standardize = true
	...
	subplot(1,2,1)
	ds = dataset
	if nlabels!="" && nlabels!=nothing && length(alabels)>0
		ds*="-"*nlabels[1]*"-"*alabels[1]
	else
		ds=ds*"-"*subclass
	end
	suptitle("$ds, seed=$seed, $model, $params, fpr=$fpr, contamination=$contamination\n
...
if basename(@__FILE__) == basename(PROGRAM_FILE)
	...
	fpr = parsed_args["fpr"]
	contamination = parsed_args["contamination"]

	f = visualize_fit(dataset, model, params, subclass, contamination, fpr, seed)
	show()
end
